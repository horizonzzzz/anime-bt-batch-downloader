...
- 支持为当前批次单独指定下载目录
- 配置页按下载器配置、站点配置、过滤规则、订阅、历史、概览等域拆分为独立工作台
- 运行时配置改为按功能域分别存储，不再依赖单一聚合 settings 文档
...
3. 对 `kisssub` 这类仍显示 `开启虫洞` 的详情页，先写入辅助 cookie，再让隐藏详情页重新加载一次
4. 解析真实的磁力链或种子地址
5. 去重重复条目
6. 把最终结果提交到当前下载器
...
- `src/lib/`：按功能域拆分的配置/存储模块、下载器适配器、运行时消息、订阅与批处理工具
