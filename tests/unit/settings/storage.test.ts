...
describe("settings storage helpers", () => {
  ...
  it("writes default app settings when storage is empty", async () => {
    ...
    await expect(fakeBrowser.storage.local.get("app_settings")).resolves.toEqual({
      app_settings: DEFAULT_SETTINGS
    })
  })
  ...
  it("persists only the app-settings document", async () => {
    ...
    const persisted = setSpy.mock.calls.at(-1)?.[0]?.app_settings as Record<string, unknown>
