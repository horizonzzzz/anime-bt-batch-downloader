...
describe("source registry", () => {
  it("resolves the source adapter for supported list pages", () => {
    ...
    expect(getSourceAdapterForPage(new URL("https://www.comicat.org/"))?.id).toBe("comicat")
    expect(getSourceAdapterForPage(new URL("https://www.comicat.org/search.php?keyword=test"))?.id).toBe("comicat")
  })
  ...
  it("resolves known adapters by id", () => {
    ...
    expect(getSourceAdapterById("comicat")?.displayName).toBe("Comicat")
  })
  ...
  it("exposes source-specific delivery mode capabilities", () => {
    ...
    expect(getSupportedDeliveryModes("comicat")).toEqual(["magnet", "torrent-file"])
  ...
  it("keeps shared source metadata aligned with the registered adapters", () => {
    expect(SOURCE_IDS).toEqual(["kisssub", "dongmanhuayuan", "acgrip", "bangumimoe", "comicat"])
  ...
  it("keeps overview accent metadata alongside the shared site configuration", () => {
    ...
    expect(SITE_CONFIG_META.comicat.overviewAccent).toBe("emerald")
  })
})
